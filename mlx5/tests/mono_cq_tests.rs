...
use mlx5::cq::{CqConfig, CqeCompressionFormat, CqeSize, Cqe};
...
fn test_mono_cq_rx_compression() {
    ...
    // Query and display device CQE compression capabilities
    let mlx5_attr = ctx.ctx.query_mlx5_device().expect("Failed to query mlx5 device");
    println!("\n=== Device CQE Compression Capabilities ===");
    println!("  flags: 0x{:x}", mlx5_attr.flags);
    println!("  CQE_128B_COMP flag: {}", (mlx5_attr.flags & 0x10) != 0);
    println!("  cqe_comp_caps.max_num: {}", mlx5_attr.cqe_comp_caps.max_num);
    println!("  cqe_comp_caps.supported_format: 0x{:x}", mlx5_attr.cqe_comp_caps.supported_format);
    println!("    HASH (0x1): {}", (mlx5_attr.cqe_comp_caps.supported_format & 0x1) != 0);
    println!("    CSUM (0x2): {}", (mlx5_attr.cqe_comp_caps.supported_format & 0x2) != 0);
    println!("    CSUM_STRIDX (0x4): {}", (mlx5_attr.cqe_comp_caps.supported_format & 0x4) != 0);
    ...
    // RX CQ with compression enabled (requires CQE size=128)
    let rx_config = CqConfig {
        cqe_size: CqeSize::Size128,
        compression_format: Some(CqeCompressionFormat::Hash),
    };
    println!("Creating RX CQ: cqe_size={:?}, compression={:?}",
             rx_config.cqe_size, rx_config.compression_format);
    let recv_cq = ctx
        .ctx
    ...
    // Debug: wait a bit for CQEs to arrive, then dump CQE
    std::thread::sleep(std::time::Duration::from_millis(100));
    println!("After 100ms:\n{}", recv_cq.debug_dump_cqe());
    ...
    while recv_completions.get() < num_ops as u64 {
        ...
        if start.elapsed() > timeout {
            // Debug: peek at CQE before panic
            let (op_own, sig, fmt) = recv_cq.debug_peek_cqe();
            println!("Timeout debug: op_own=0x{:02x}, sig=0x{:02x}, format={}", op_own, sig, fmt);
            panic!(
                "Timeout: recv={}, expected={}",
...
/// Test CQE compression API and detection logic.
///
/// This test verifies that:
/// 1. CQE compression can be enabled via `CqConfig::compression_format`
/// 2. `is_compression_enabled()` reports correct state
/// 3. The detection logic for compressed CQEs (format=3) is in place
///
/// Note: CQE compression (mini CQEs) may not be triggered in typical RDMA verbs
/// scenarios. Hardware triggers compression under specific conditions (PCIe back
/// pressure, specific traffic patterns) that are difficult to reproduce in tests.
/// See UCX and DPDK implementations for reference - both have CQE zipping disabled
/// by default due to its workload-specific nature.
#[test]
fn test_cqe_compression_high_load() {
    let ctx = match TestContext::new() {
        Some(ctx) => ctx,
        None => {
            eprintln!("Skipping test: no mlx5 device available");
    ...
    let mlx5_attr = ctx.ctx.query_mlx5_device().expect("Failed to query mlx5 device");
    println!("\n=== High-Load CQE Compression Test ===");
    println!("Device flags: 0x{:x}", mlx5_attr.flags);
    println!("CQE comp max_num: {}", mlx5_attr.cqe_comp_caps.max_num);
    ...
    // Create CQs - RX with compression, TX without
    let send_cq = ctx
        .ctx
        .create_mono_cq::<RcQpForMonoCq<u64>, _>(4096, |_cqe: Cqe, _entry: u64| {})
        .expect("Failed to create send MonoCq");

    // CQE compression requires:
    // 1. CQE size = 128 bytes
    // 2. compression_format specified (HASH, CSUM, or CSUM_STRIDX)
    let rx_config = CqConfig {
        cqe_size: CqeSize::Size128,
        compression_format: Some(CqeCompressionFormat::Hash),
    };
    println!("Creating RX CQ with CQE size={:?}, compression={:?}",
             rx_config.cqe_size, rx_config.compression_format);
    let recv_cq = ctx
        .ctx
        .create_mono_cq_with_config::<RcQpForMonoCq<u64>, _>(4096, recv_callback, &rx_config)
        .expect("Failed to create recv MonoCq with compression");
    ...
    // Larger buffers for high-load test
    let buf_size = 1024 * 1024; // 1MB
    let mut send_buf = AlignedBuffer::new(buf_size);
    let mut recv_buf = AlignedBuffer::new(buf_size);
    ...
    // High-load test: many small operations to create PCIe pressure
    let num_ops = 2048;
    let chunk_size = 64u32; // Small chunks

    println!("Posting {} recv buffers...", num_ops);

    // Post all receive buffers first
    for i in 0..num_ops {
        let offset = ((i as u32 * chunk_size) % (buf_size as u32 - chunk_size)) as u64;
        qp1.borrow_mut()
            .recv_builder(i as u64)
            .expect("recv_builder failed")
            .sge(recv_buf.addr() + offset, chunk_size, recv_mr.lkey())
    ...
    println!("Sending {} RDMA WRITE IMM operations in burst...", num_ops);

    // Send all operations in rapid succession (burst mode)
    let start = std::time::Instant::now();
    for i in 0..num_ops {
        let offset = ((i as u32 * chunk_size) % (buf_size as u32 - chunk_size)) as u64;
        let _ = qp2
    ...
    // Wait briefly to let CQEs accumulate before polling
    // This creates artificial back pressure that might trigger compression
    std::thread::sleep(std::time::Duration::from_millis(50));

    // Check what's pending before we start polling
    let (peek_op_own, peek_sig, peek_fmt) = recv_cq.debug_peek_cqe();
    println!(
        "Before poll - CQE peek: op_own=0x{:02x}, sig=0x{:02x}, format={}",
        peek_op_own, peek_sig, peek_fmt
    );

    // Wait for completions
    let timeout = std::time::Duration::from_secs(10);
    let mut format_samples: Vec<(u8, u8, u8)> = Vec::new();
    let mut last_sample_count = 0u64;

    while recv_completions.get() < num_ops as u64 {
        // Sample format bits before polling (every 100 completions)
        if recv_completions.get() > last_sample_count + 100 {
            format_samples.push(recv_cq.debug_peek_cqe());
            last_sample_count = recv_completions.get();
        }
    ...
    let throughput = num_ops as f64 / elapsed.as_secs_f64() / 1_000_000.0;

    println!("\n=== Results ===");
    println!("  Operations: {}", num_ops);
    println!("  Time: {:?}", elapsed);
    println!("  Throughput: {:.2} Mops/s", throughput);
    println!("  Compression enabled: {}", recv_cq.is_compression_enabled());
    println!("  Compressed CQEs detected: {}", recv_cq.compressed_cqe_count());

    if !format_samples.is_empty() {
        println!("  CQE format samples (op_own, signature, format_bits):");
        for (i, (op_own, sig, fmt)) in format_samples.iter().take(5).enumerate() {
            println!(
                "    Sample {}: op_own=0x{:02x}, sig=0x{:02x}, format={}",
                i, op_own, sig, fmt
            );
        }
    }

    if recv_cq.compressed_cqe_count() > 0 {
        println!("  SUCCESS: CQE compression is working!");
    } else {
        println!("  Note: No compressed CQEs detected (PCIe may not be under pressure)");
    }
}
