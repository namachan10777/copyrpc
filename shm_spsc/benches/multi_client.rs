...
fn run_bench_shared(b: &mut criterion::Bencher, depth: u32) {
    let name = format!("/shm_mc_shared_{}", Uuid::now_v7());
    ...
    unsafe {
        let mut server = shm_spsc::mpsc_shared::Server::<u64, u64>::create(
            &name,
            NUM_CLIENTS as u32,
            RING_DEPTH,
        )
        ...
        for client_idx in 0..NUM_CLIENTS {
            ...
            client_handles.push(thread::spawn(move || {
                ...
                if depth <= 1 {
                    let mut client =
                        shm_spsc::mpsc_shared::SyncClient::<u64, u64>::connect_sync(&name_clone)
                            .unwrap();
                    for _ in 0..100 {
                        client.call_blocking(0).unwrap();
                    }
                    ...
                    let count = std::cell::Cell::new(0u64);
                    let mut client =
                        shm_spsc::mpsc_shared::Client::<u64, u64, (), _>::connect(
                            &name_clone,
                            |(), _resp| {
                                count.set(count.get() + 1);
                            },
...
fn bench_multi_client(c: &mut Criterion) {
    ...
    // Shared MPSC
    group.bench_function("shared_depth_1", |b| run_bench_shared(b, 1));
    group.bench_function("shared_depth_4", |b| run_bench_shared(b, 4));
