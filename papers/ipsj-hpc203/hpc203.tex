...
benchkv評価では，提案方式は2ノードおよび4ノードでUCXおよびdirect connectionを上回る一方，32ノードでは両方式を下回った．ただし同条件でAdaptive Batch Holdはno batch hold比で最大1.44倍の改善を示した．
\end{abstract}
...
数値計算においては，MPI等の集団通信最適化研究の結果この問題は緩和されているが\cite{weihangjiangHighPerformanceMPI22004,sugiharaDesignLocalityawareMPIIO2020}，
ストレージ等のHPCアプリを補助するミドルウェアでの通信では，アクセス先の偏りや通信相手を固定的に仮定しにくく，これらの最適化を直接適用することは難しい．
MPI-IOのようなアクセスパターンの仮定が可能なミドルウェアでは高いスケーラビリティを達成する手法が提案されている一方\cite{sugiharaDesignLocalityawareMPIIO2020,sugiharaDesignDirectRead2020}，
HPCアプリではPOSIXインターフェースを備える並列ファイルシステムを仮定したアプリも多い．

天文データ解析，機械学習，大気シミュレーションといった高いストレージ性能を要求するアプリへの回答として，
富岳ではLLIOが導入され並列ファイルシステムの性能を補完している\cite{akimotoFileSystemPower,nukariyaHPCAIInitiatives}．
このような計算ノードでの一時的なストレージミドルウェアはad-hocファイルシステムとして研究が行われている\cite{brinkmannAdHocFile2020,tatebeGfarmBBGfarm2020,tatebeCHFSParallelConsistent2022,vefGekkoFSTemporaryDistributed2018,wangBurstFSDistributedBurst2016,zhengDeltaFSExascaleFile2015,zhengDeltaFSScalableNogroundtruth2021}．
...
このRC接続の状態管理がネットワークインターフェースにおいてスケーラビリティの課題を抱えていることが報告されている\cite{chenScalableRDMARPC2019,kalia2019DatacenterRPCsCan,kong2023UnderstandingRDMAMicroarchitecture,zhangRevisitingUnderlyingCauses2024}．
また，ミドルウェアは計算を補助する役割であるため，通信のためのメモリリソースが計算用メモリを過度に圧迫しないことが望ましい．
...
本稿の構成は以下の通りである．
まず，背景節（\ref{sec:background}）でad-hocファイルシステムとRDMA通信における課題を整理する．
次に，関連研究節（\ref{sec:related-work}）を述べ，設計節（\ref{sec:design}）で提案アーキテクチャと通信プロトコルを示す．
実験節（\ref{sec:evaluation}）でbenchkvによる評価結果を示し，おわりに（\ref{sec:conclusion}）で結論と今後の課題を述べる．
...
そのため，1回のポーリングで処理できるCQE数が小さい状態では，この固定コストの償却が不十分となり，スループットが低下しやすい．
本稿でいうbatching崩壊は，まさにこの受信側の固定コスト償却が崩れる現象を指す．
...
coalescingベース同期と送受信スケジューリングを組み合わせることで，同期オーバーヘッドを抑えつつ公平な接続利用を達成できることを示した\cite{monga2021BirdsFeatherFlock}．
...
本稿では，イベントループ1回あたりの到着リクエスト数の分布により，
小バッチ状態と大バッチ状態の双方へ遷移しうる双安定性が生じると捉える．
小バッチ状態ではローカル処理の固定コスト（CQポーリング，IPC到達点チェック，送信可能範囲チェック）が相対的に支配的になるため，
到着率に応じて保持時間を適応的に更新する．
この現象を簡易モデルでみると，ランク数増加で低性能側へ安定しやすくなることが分かる．
クライアント1つあたりの発行率を$\lambda_c$，ランクあたりクライアント数を$C$，
ランク数を$N_{\mathrm{rank}}$とすると，1つのリモート接続へ到着する平均到着率は
\begin{equation}
\lambda_{\mathrm{conn}} \approx \frac{C\lambda_c}{N_{\mathrm{rank}}-1}
\end{equation}
と近似できる（宛先一様分布）．
イベントループの固定処理時間を$T_{\mathrm{fix}}$とすると，1ループで観測される到着数の期待値は
\begin{equation}
\mu = \lambda_{\mathrm{conn}}T_{\mathrm{fix}}
\end{equation}
であり，$N_{\mathrm{rank}}$の増加とともに$\mu$は低下する．
$\mu<1$の領域では空ループや小バッチループが増加し，固定コスト償却が崩れるため低性能側へ安定化しやすい．

制御状態は，保持キュー長$B$，保持開始からの経過時間$\Delta t$，
目標バッチサイズ$B_{\mathrm{target}}$，保持時間$T_{\mathrm{hold}}$，
および到着率推定$\hat{\lambda}$で表す．
本実装では$B_{\mathrm{target}}=N_{\mathrm{rank}}$とする．

LocustaRPC応答到着時は保持キューへ追加し，保持キューが非空のとき，
次の条件のいずれかでフラッシュする．
\begin{equation}
B \geq B_{\mathrm{target}}
\;\;\vee\;\;
\Delta t \geq T_{\mathrm{hold}}
\;\;\vee\;\;
outstanding = 0
\end{equation}
ここで$outstanding$は未完了のリモート要求数である．

フラッシュ時には，実フラッシュサイズ$B_{\mathrm{flush}}$と保持時間$\Delta t_{\mathrm{flush}}$から
観測到着率$\lambda_{\mathrm{obs}}$を求める．
\begin{equation}
\lambda_{\mathrm{obs}} = \frac{B_{\mathrm{flush}}}{\Delta t_{\mathrm{flush}}}
\end{equation}
この到着率はEWMAで平滑化する（実装では係数$0.125$）．
\begin{equation}
\hat{\lambda}_{k+1} = 0.875\hat{\lambda}_{k} + 0.125\lambda_{\mathrm{obs}}
\end{equation}
次の保持時間は「目標バッチを形成するのに必要な時間」として
\begin{equation}
T_{\mathrm{hold}} \leftarrow \frac{B_{\mathrm{target}}}{\hat{\lambda}}
\end{equation}
で更新し，実装上は$[0.5\mu s,100\mu s]$へクランプする．

この更新則により，到着率が低いと$T_{\mathrm{hold}}$は増加し，到着率が高いと$T_{\mathrm{hold}}$は減少する．
すなわち，小バッチ状態では保持時間を長くしてバッチ形成を促進し，
大バッチ状態では保持時間を短くして待ち時間を抑える適応制御になる．
本制御が必要となる現象の説明と有効性の評価は，実験結果（\ref{fig:benchkv-batch-hold}）で述べる．
...
評価ノードはfern03およびfern04であり，各ノードはIntel Xeon Gold 6530（32コア）を搭載する．
ネットワークはInfiniBandで接続し，NICはMellanox ConnectX-7を用いた．
評価時はNUMAノードが1つの構成を用い，コア0および1はIRQ処理に割り当て，ベンチマーク実行には使用しない．

\subsection{実験手法}

提案手法の効果は，通信ライブラリ単体ではなく，ベンチマーク用Key-Value Storeであるbenchkvを用いたend-to-end測定で評価する．
benchkvは，keyをrank idとユーザキーの組で管理する単純なKVSであり，アプリケーションロジックの複雑さを抑えて通信層の影響を観測することを目的とする．
...
\begin{figure}[t]
	\centering
	\includegraphics[width=0.95\linewidth]{figures/benchkv_throughput.pdf}
	\caption{benchkvにおけるノード数に対する総スループット（MIOPS）}
	\label{fig:benchkv-throughput}
\end{figure}

\begin{figure}[t]
	\centering
	\includegraphics[width=0.95\linewidth]{figures/benchkv_batch_hold.pdf}
	\caption{Adaptive Batch Hold有無の比較（benchkv総スループット）}
	\label{fig:benchkv-batch-hold}
\end{figure}

図\ref{fig:benchkv-throughput}に，benchkvの総スループットを示す．
...
図\ref{fig:benchkv-batch-hold}に，提案方式におけるAdaptive Batch Holdの有無を比較した結果を示す．
no batch holdに対してbatch hold（10$\mu$s）は，2ノードで13.97から14.86 MIOPS（1.06倍），
...
全体として中大規模条件では保持時間制御により小バッチ状態への収束を抑制できることが分かる．
