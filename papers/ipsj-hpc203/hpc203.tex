...
benchkv評価では，提案方式は2ノードおよび4ノードでUCXおよびdirect connectionを上回るスループットを示し，32ノード条件ではAdaptive Batch Holdによりno batch hold比で最大1.44倍の改善を得た．
\end{abstract}
...
低バッチ状態と高バッチ状態の双方へ遷移しうる双安定性が生じると捉える．
低バッチ状態ではローカル処理の固定コスト（CQポーリング，IPC到達点チェック，送信可能範囲チェック）が相対的に支配的になるため，
...
すなわち，低バッチ状態では保持時間を長くしてバッチ形成を促進し，
高バッチ状態では保持時間を短くして待ち時間を抑える適応制御になる．
...
この8ノードでの低下は，リモート通信経路におけるbatching崩壊が生じ，CQEバッチが十分に形成されない低均衡状態へ遷移したためと解釈できる．
