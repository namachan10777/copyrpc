...
本稿で用いる送信バッファの論理フォーマットを表\ref{tab:send-buffer-format}に示す．

\begin{table}[t]
	\caption{送信バッファの論理フォーマット}
	\label{tab:send-buffer-format}
	\centering
	\begin{tabular}{p{0.27\linewidth}|p{0.63\linewidth}}
		\hline
		領域/項目 & 役割 \\
		\hline
		Slot Header & call\_id，generation，状態（Empty/ReqReady/RespReady），要求長，応答長を保持する固定長メタデータ \\
		Data Area & 要求または応答の可変長ペイロード本体 \\
		Producer Position & クライアント側が書き込み完了として公開した末尾位置 \\
		Consumer Position & デーモン側が送信済み/解放済みとして進めた位置 \\
		Generation Counter & リング折返し時の世代識別子（同一オフセット再利用時の取り違え防止） \\
		\hline
	\end{tabular}
\end{table}
...
第1段階では，クライアントが原子的なtail更新で書き込み区間を確保する．
この時点では他プロセスから当該スロットは不可視である．
第2段階で，クライアントはSlot HeaderとData Areaを書き込み，最後に状態ビットを公開状態へ遷移させる．
デーモンは公開済み状態のスロットのみを対象に送信範囲を確定し，WQEを生成する．
