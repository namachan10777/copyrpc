...
for E in 1 2 4 8 16 32 64 128 256 512 1024 2048 4096; do
  for QD in 1 8 256; do
    echo "=== copyrpc 1-to-1 QD=$QD E=$E ==="
    timeout 120 mpirun --hostfile "$PBS_NODEFILE" -np 2 --map-by node --bind-to none "$BENCH" \
      -d 10 -r 3 -s 32 \
      -o "$OUTDIR/copyrpc_1to1_qd${QD}_e${E}.parquet" \
      --affinity-mode multinode --affinity-start 47 \
      copyrpc --ring-size 1048576 one-to-one -e $E -i $QD -t 1 \
      || echo "FAILED: copyrpc E=$E QD=$QD"
  done
done

echo "=== copyrpc EP scaling benchmarks completed ==="
