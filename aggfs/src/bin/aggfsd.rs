//! aggfsd: Filesystem daemon binary.
//!
...
#[derive(Parser, Debug)]
#[command(name = "aggfsd", about = "aggfs daemon")]
struct Cli {
    ...
    /// If not set, uses /tmp/aggfs_<rank>_<daemon>.
    #[arg(long)]
...
fn update_file_size(
    file_metadata: &mut std::collections::HashMap<u64, aggfs::InodeHeader>,
    store: &PmemStore,
    path_hash: u64,
    chunk_index: u32,
    offset: u64,
...
fn run_daemon(
    ...
    let mut file_metadata: std::collections::HashMap<u64, aggfs::InodeHeader> =
        ...
        if let Some(ctx) = copyrpc_ctx_ref {
            ctx.poll(|pending_op, resp_data| {
                ...
                match pending_op {
                    ...
                    PendingOp::Stat(token) => {
                        if resp_data.len()
                            >= std::mem::size_of::<aggfs::message::RemoteStatResponse>()
                        {
                            let stat_resp: aggfs::message::RemoteStatResponse = unsafe {
                                std::ptr::read_unaligned(resp_data.as_ptr()
                                    as *const aggfs::message::RemoteStatResponse)
                            };
                ...
                match remote_req {
                    ...
                    RemoteRequest::Stat(stat_req) => {
                        match file_metadata.get(&stat_req.path_hash) {
                            Some(header) => {
                                let stat_resp = aggfs::message::RemoteStatResponse {
                                    status: 0,
                                    header: *header,
                                };
                                let resp_bytes = unsafe {
                                    std::slice::from_raw_parts(
                                        &stat_resp as *const aggfs::message::RemoteStatResponse
                                            as *const u8,
                                        std::mem::size_of::<aggfs::message::RemoteStatResponse>(
                                        ),
                                    )
                            ...
                            None => {
                                let stat_resp = aggfs::message::RemoteStatResponse {
                                    status: -2, // NotFound
                                    header: aggfs::InodeHeader::default(),
                                };
                                let resp_bytes = unsafe {
                                    std::slice::from_raw_parts(
                                        &stat_resp as *const aggfs::message::RemoteStatResponse
                ...
                FsRequest::Write {
                    ...
                    if target == my_global_id {
                        ...
                        if let Some(buf_ptr) = extra_buf_ptrs.get(client_idx).copied().flatten() {
                            let data = unsafe {
                                std::slice::from_raw_parts(
                                    buf_ptr.add(aggfs::message::DATA_AREA_OFFSET),
                                    len as usize,
                                )
                            };
                        ...
                        if let Some(&ep_idx) = remote_rank_to_ep_idx.get(&target_node) {
                            ...
                            let remote_req = RemoteRequest::Write(RemoteWriteReq {
                                ...
                                client_addr: mr_info.mr.addr() as u64
                                    + aggfs::message::DATA_AREA_OFFSET as u64,
                            });
...
fn main() {
    ...
    let data_capacity = cli.extra_buffer_size as usize - aggfs::message::DATA_AREA_OFFSET;
    if data_capacity < cli.chunk_size {
        eprintln!(
            ...
            aggfs::message::DATA_AREA_OFFSET,
            data_capacity,
            cli.chunk_size,
            cli.chunk_size + aggfs::message::DATA_AREA_OFFSET,
        );
        std::process::exit(1);
    }
    ...
    for d in 0..num_daemons {
        let pmem_path = match &cli.pmem_path {
            Some(p) => format!("{}_{}", p, d),
            None => format!("/tmp/aggfs_{}_{}", rank, d),
        };
