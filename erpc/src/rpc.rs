...
use crate::packet::{PKT_HDR_SIZE, PktHdr, PktType, SM_PKT_HDR_SIZE, SmPktHdr, SmPktType};
...
impl<U: 'static> Rpc<U> {
    ...
    fn send_sm_packet(&self, session: SessionHandle, sm_hdr: &SmPktHdr) -> Result<()> {
        ...
        let av = {
            ...
            let ah = sess
                .ah
                .as_ref()
                .ok_or(Error::SessionNotConnected(session.session_num()))?;
    ...
    pub fn call(
        ...
        if likely(num_pkts == 1) {
            ...
            // Post send
            {
                ...
                let addr = send_buffers.slot_addr(buf_idx);
                self.transport
                    .post_send_raw(av, addr, pkt_len, lkey, entry)?;
            }
    ...
    fn send_response(
        ...
        let av = {
            ...
            let ah = sess
                .ah
                .as_ref()
                .ok_or(Error::SessionNotConnected(session_num))?;
            UdTransport::ah_to_av(ah)
        };
        ...
        // Phase 4: Send - use buffered send for all responses
        {
            ...
            let lkey = send_buffers.lkey();
            self.transport
                .post_send_raw(av, addr, total_len as u32, lkey, entry)?;
        }
        // Doorbell is batched in run_event_loop_once()
    ...
    fn process_recv(&self, buf_idx: usize, byte_cnt: u32, batch_ts: u64) -> Result<()> {
        ...
        let hdr = {
            ...
            if (byte_cnt as usize) < GRH_SIZE + PKT_HDR_SIZE {
                ...
                drop(recv_buffers);
                self.recv_buffers.borrow_mut().free(buf_idx);
                self.posted_recv_count
                    .set(self.posted_recv_count.get().saturating_sub(1));
    ...
    fn batch_repost_recv_buffers(&self) {
        ...
        for &(idx, addr, capacity, lkey) in buf_infos.iter() {
            ...
            if self
                .transport
                .post_recv_raw(addr, capacity, lkey, entry)
                .is_ok()
            {
                count += 1;
            }
        ...
        // Update posted count
        self.posted_recv_count
            .set(self.posted_recv_count.get() + count);
    ...
    fn handle_response_single_packet(
        ...
        let pending = {
            ...
            let sess = sessions
                .get_mut(handle)
                .ok_or(Error::SessionNotFound(dest_session))?;
            ...
            if let Some(wheel_slot) = sslot.timer_wheel_slot {
                self.timing_wheel
                    .borrow_mut()
                    .cancel_fast(wheel_slot, hdr.req_num());
            }
    ...
    fn handle_response_multi_packet(
        &self,
        hdr: &PktHdr,
        payload: &[u8],
        batch_ts: u64,
    ...
    fn process_timeouts(&self, batch_ts: u64) {
        ...
        self.timing_wheel
            .borrow_mut()
            .advance_into(batch_ts, &mut expired);
                    ...
                    if sslot.req_num == entry.req_num && !sslot.is_free() {
                        ...
                        if sslot.retries > self.config.max_retries {
                            ...
                            sslot
                                .user_data
                                .as_ref()
                                .map(|p| (p.single_buf_idx, p.multi_buf_indices.clone()))
                        }
                    } else {
                        None
                    }
